...
def loadMedNISTData(root_dir):
    ...
    image_width, image_height = PIL.Image.open(
        image_files[class_names[0]][0]).size
...
def loadMNIST(split_tuple: tuple, image_files: dict, mask_imgs: list, sprite_chances: tuple, maskmax: bool):
    ...
    if train_frac + val_frac + test_frac > 1:
        raise ValueError(
            f"Proportions of class {class_name} ({train_frac},{val_frac},{test_frac}) sum to greater than 1.")
    ...
    val_indices = split_indices[int(
        dat_len*train_frac):int(dat_len*train_frac)+int(dat_len*val_frac)]
    test_indices = split_indices[int(dat_len*train_frac)+int(dat_len*val_frac):int(
        dat_len*train_frac)+int(dat_len*val_frac)+int(dat_len*test_frac)]
    ...
    print(
        f"For {class_name} added {len(add_train)} train, {len(add_val)} val, {len(add_test)} test.")
...
def loadWMH(split_tuple: tuple, init_path: str, folder: Path, supervised: bool):
    ...
    for file in getPatients(init_path / "train"):
        ...
        # (t1_path,flair_path,mask_path)
        train_files.append((flair_path, mask_path))
    ...
    for file in getPatients(init_path / "test"):
        ...
        mask_path = file / mask_name
        # (t1_path,flair_path,mask_path)
        test_files.append((flair_path, mask_path))
    print(f"Found {len(test_files)} patients.")
    train_frac, val_frac = split_tuple
    if train_frac + val_frac > 1:
        raise ValueError(
            f"Proportions ({train_frac},{val_frac}) sum to greater than 1.")
    ...
    def load_files(files, outputpath):
        ...
        if not os.path.exists(outputpath/'datinfo.gz'):
            for fileimg in tqdm(files):
                # nib.load(fileimg[1]).get_fdata()
                flair = nib.load(fileimg[0]).get_fdata()
                ...
                # nib.load(fileimg[2]).get_fdata()
                mask = nib.load(fileimg[1]).get_fdata()
                mask = mask[:, :, slices]
    ...
    filtered_test_files = [f for f in itertools.chain.from_iterable(
        itertools.zip_longest(testdict["0"], testdict["1"])) if f != None]

    print(
        f"Test data has {numtestnorm} normal and {numtestabnorm} abnormal cases.")
    ...
    if supervised:
        ...
        filtered_train_files = [f for f in itertools.chain.from_iterable(itertools.zip_longest(
            traindict["0"][:num_train_unsup], traindict["1"][:num_train_sup])) if f != None]

        # want to alternate between normal and abnormal cases to avoid having no 1 values (metric failure)
        filtered_val_files = [f for f in itertools.chain.from_iterable(itertools.zip_longest(
            traindict["0"][num_train_unsup:], traindict["1"][num_train_sup:])) if f != None]
        ...
        print(
            f"Putting {num_train_files} normal cases in training set ({num_unsup-num_train_files} in validation set).")

        filtered_train_files = filtered_t_files[:num_train_files]
        filtered_val_files = [f for f in itertools.chain.from_iterable(itertools.zip_longest(
            filtered_t_files[num_train_files:num_unsup], filtered_t_files[num_unsup:])) if f != None]
    ...
    def __getitem__(self, index):
        ...
        if type(index) == int:
            try:
                ...
                dat = read_np(
                    curr_dir / self.image_files[index].replace("\\", "/"))
            ...
            try:
                dat = np.asarray(
                    [read_np(curr_dir / self.image_files[i]) for i in r[index]])
            except:
                dat = np.asarray(
                    [read_np(curr_dir / self.image_files[i].replace("\\", "/")) for i in r[index]])
...
def loadData(exp_name):
    ...
    if exp_name == "mnist_usp":
        ...
        root_dir = Path(
            r"E:\SCHOOL\FINISHED\CS274E\CS-274E-Brain-Segmentation\WMH-dataset")
    ...
    if type(train_file_name) is str:
        if all_files_exist([folder/train_file_name, folder/val_file_name, folder/test_file_name]):
            ...
            if "mnist" in exp_name:
                ...
                train_x, val_x, test_x = loadMNIST(
                    dat_split, image_files, mask_imgs, sprite_chances, maskmax)
            elif "wmh" in exp_name:
                train_x, val_x, test_x = loadWMH(
                    dat_split, root_dir, folder, is_supervised)
        ...
        print(
            f"Loaded {len(train_x)} train, {len(val_x)} val, {len(test_x)} test.")
        ...
        for i in range(len(train_file_name)):
            if all_files_exist([folder/train_file_name[i], folder/val_file_name[i], folder/test_file_name[i]]):
                ...
                if "mnist" in exp_name:
                    ...
                    mask_imgs = loadDSprites()
                    train_x, val_x, test_x = loadMNIST(
                        dat_split, image_files, mask_imgs, sprite_chances[i], maskmax)
                elif "wmh" in exp_name:
                    train_x, val_x, test_x = loadWMH(
                        dat_split, root_dir, folder, is_supervised[i])
        ...
        print(
            f"Loaded {len(full_train_x)} train, {len(full_val_x)} val, {len(full_test_x)} test total.")
