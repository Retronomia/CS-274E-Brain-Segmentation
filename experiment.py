...
def loadobjective(trial):
    ...
    folder_name = study_name + '-' + \
        str(loaderdict['trial_num'])+"-"+exp_name+"-" + \
        model_name+'-'+"_".join(args_loss_name)  # +'-'+time_stamp
...
if __name__ == "__main__":
    ...
    parser.add_argument('--exp_name', type=str,
                        default='wmh_usp', help='experiment name')
    parser.add_argument('--model_name', type=str,
                        default='AutoEnc', help='model name')
    parser.add_argument('--loss_name', '--list', nargs='+',
                        default=['L1_Loss'], help='loss name')
    parser.add_argument('--tqdm', action='store_true', help='use tqdm')
    parser.add_argument('--comment', type=str, default='None',
                        help='reminder print out for logs')
    args = parser.parse_args()
    args_model_name = args.model_name
    args_exp_name = args.exp_name
    args_loss_name = tuple(args.loss_name)
    args_use_tqdm = args.tqdm
    print("Comment:", args.comment)

    study_name = ''.join(random.choices(
        string.ascii_lowercase + string.digits, k=5))

    vae_study = optuna.create_study(
        direction="minimize", study_name=study_name)

    start = time.time()
    vae_study.optimize(loadobjective, n_trials=1, timeout=60 *
                       60*24, gc_after_trial=True, show_progress_bar=False)

    pruned_trials = vae_study.get_trials(
        deepcopy=False, states=[TrialState.PRUNED])
    complete_trials = vae_study.get_trials(
    ...
    best_folder = study_name + '-' + \
        str(trial.number)+"-"+args_exp_name + \
        "-"+args_model_name+'-'+"_".join(args_loss_name)
